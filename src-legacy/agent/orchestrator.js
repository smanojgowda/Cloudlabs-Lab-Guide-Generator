...
export async function stopAndGenerate(opts = {}) {
  ...
  if (steps.length === 0) {
    // Re-activate so user can keep capturing
    if (process.env.DESKTOP_MODE === 'true') {
      startDesktopRecording();
    }
    sessionActive = true;
  ...
  // Re-activate session so user can continue capturing more steps
  if (process.env.DESKTOP_MODE === 'true') {
    startDesktopRecording();
  }
  sessionActive = true;
  logger.info('Session re-activated — you can continue capturing steps.');
